    ...
    <div class="">
        <div class="page-title">
            <div class="title_left">
                <h3>{{ __('all.manage_items') }}: <small>{{ $feature->title }}</small></h3>
            </div>
        </div>
        ...
        <div class="modal fade bs-example-modal-lg lang-modal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <form id="lang-form" method="post" action="{{ route('item.save', [app()->getLocale(), $feature->feature_id]) }}">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span></button>
                            <h4 class="modal-title" id="myModalLabel">{{ __('all.add_edit_item') }}</h4>
                        </div>
                        <div class="modal-body ">
                            <div class="row">
                                ...
                                <div class="form-group col-md-6 price">
                                    <label for="image">{{ __('all.price') }}</label>
                                    <input type="number" id="price" min="0" step="1" class="form-control" name="price" placeholder="{{ __('all.price') }}"/>
                                </div>
                                ...
                                <div class="form-group col-md-6 preview_image">
                                    <label for="image">{{ __('all.preview_image') }}</label>
                                    <input type="file" id="preview_image" class="form-control" name="preview_image">
                                </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <input type="submit" class="btn btn-primary" value="{{ __('all.save') }}">
                        </div>
                    </div>
                </form>
            </div>
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                table = $('#languageDatatable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('items.datatables.list', app()->getLocale()) }}",
                    columns: [
                ...
                $(document).on('click', '.edit', function (e) {
                    ...
                    CKEDITOR.instances.text.setData(decodeHTMLEntities(translation.description));
                    sTable = 'main';
                    langModal.modal('show');
                });
                ...
                $('body').on('click', '.delete', function () {
                    var data = table.row($(this).parents('tr')).data();
                    var url = "{{ route('item.destroy', [app()->getLocale(), '***']) }}";
                ...
                $('#languageDatatable tbody').on('click', 'td.details-control', function () {
                    ...
                    if (row.child.isShown()) {
                        ...
                        childTable = $('#in' +bid).DataTable({
                            ajax: {
                                url: "{{ route('items.detail.grid', app()->getLocale()) }}?id=" + bid,
                                "type": "GET"
                            },
                            columns: [
            ...
            function initializeSubEditable() {
                $('.editable__field').editable({
                    mode: 'inline',
                    url: "{{route('update.item.field', app()->getLocale())}}",
